    ...
    #subunits list for the current receptor
        ...
        for x, sub in enumerate(r_sub[0]):
           # print(f'{r} - {sub}')
            ...
            #KStest = calc_KS(mc_mask1, mc_mask2)
            #KStest = KStest[0]
            KS,KSci_low,KSci_up = bootstrap_KS(mc_mask1, mc_mask2, n_samples=n_samples,alpha=alpha)
            KStest_all.append(KS)
            KS_up.append(KSci_up)
            KS_low.append(KSci_low)
         ...
         #### string cohens D and CI's 
            ...
            #### string cohens D and CI's 
            KS_row=[]
            format="{KS:.2f} ({KS_lower: .2f} - {KS_upper: .2f})"

            for k in range(0,len(KStest_all)):
                KS_row.append(format.format(KS=KStest_all[k], KS_lower=KS_low[k], KS_upper=KS_up[k]))
            KS_row=pd.DataFrame(KS_row)
    ...
    ######################################
     ...
     #  KS_row = []
       ...
       # KStest_df = pd.DataFrame(KS_row)
        ...
        subunit_outputs = pd.concat([pctDifs_df, dval_row, KS_row], axis=1)
...
def calc_KS(ks1,ks2):
    KS = stats.kstest(ks1, ks2,  alternative ='two-sided', mode = 'auto')
    return(KS[0])

def get_boot_KS(ks1,ks2):
    idx1 = np.random.randint(0,ks1.shape[0],ks1.shape[0])
    idx2 = np.random.randint(0,ks2.shape[0],ks2.shape[0])
    return(calc_KS(ks1[idx1],ks2[idx2]))

def bootstrap_KS(ks1,ks2,n_samples=10,alpha=0.05):
    #alphas = np.array([alpha/2,1-alpha/2])
    KS = calc_KS(ks1,ks2)
    KSci = sorted(Parallel(n_jobs=-2,verbose=1)(delayed(get_boot_KS)(ks1,ks2)for i in range(n_samples)))
    KSci_low = np.percentile(KSci, 2.5)
    KSci_up = np.percentile(KSci, 97.5)
    return(KS,KSci_low,KSci_up)